...
The gem tests are run against 1.9.1, 1.9.2, and 1.9.3, so Serious should run fine at least
on those interpreters - it is highly recommended to go with 1.9.3 though.
...
You're welcome to add the real tags to your posts. Just define the necessary
section at your post metadata:
...
After this, you may to use the <code>Serious::Article#tags</code> method to
fetch the tags of the post. More than all, the <code>Serious::Article.tags</code>
method is available and designed to make your own tag cloud easily:

    Serious::Article.tags # => {"friendship"=>1, "magic"=>1}

The <code>Serious::Article.all</code> method is also modified to perform
tagged search:

    Serious::Article.all :tag => "magic" # => [ ... ]

Your visitors may to visit the tagged archives by URL:
<code>/archives/:tag</code>.

=== Internationalization

It is possible to perform basic internationalization of the
Serious-driven blog. Just add the <code>locales</code> directory
into your code and create the <code>*.yml</code> files with following
content:
...
Yes, this is very similar to Rails I18n (actually, the same `i18n` gem
is used). The entire list of translatable strings is available at
`lib/serious/locale/en.yml` file in the Serious repository.

These translations can be used both to render traslated strings at
views:

    t('serious.views.pages')

and to use 'em at your customizations in Ruby:

    I18n.t('serious.views.pages')

The default locale is <code>:en</code>. This can be changed during the
Serious initialization phase by implicit assignment:
...
Serious is made by {fascinated people}[https://github.com/eveel/serious/contributors]
and proudly released under the MIT license:
